<template>
  <div>
    ...
    <p class="file-tips">
      图片尺寸681{{ sizeWidth }}*831{{ sizeHeight }}像素，支持{{ accept }}格式
    </p>
  </div>
</template>
...
export default {
  ...
  props: {
    ...
    isLimitType: {
      // 限制尺寸类型，0 不限制，1 等宽等高，2 比例宽高，3 小于宽高，4 大于宽高，5 宽<高，6 宽>高
      type: Number,
      default: 0,
    },
    isHideUpload: {
      ...
      const isValidate = new Promise(function (resolve, reject) {
        ...
        img.onload = function () {
          ...
          switch (this.isLimitType) {
            case 1:
              valid = img.width === this.sizeWidth && img.height === this.sizeHeight;
              break;
            case 2:
              valid = img.width / img.height === this.sizeWidth / this.sizeHeight;
              break;
            case 3:
              valid = img.width < this.sizeWidth && img.height < this.sizeHeight;
              break;
    ...
    handleUploadForm(param) {
      ...
      this.getBase64(param.file)
        .then((res) => {
          ...
          // 向后端发送数据
          _this.$axios
            .post(this.$constants.requireUrl.uploadFile, imgData)
            .then((res) => {
              this.isUpload = false;
              if (res) {
